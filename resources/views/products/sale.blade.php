<!-- resources/views/products/sale.blade.php -->
@extends('layouts.app')

@section('title', 'Sale - STEP UP')

@section('content')
<div class="bg-white pt-24 pb-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center border-b border-gray-200 pb-6 mb-8">
            <h1 class="text-3xl font-bold">Sale Items</h1>
            <div class="flex items-center">
                <form action="{{ route('products.sale') }}" method="GET" class="flex">
                ...
                <select name="sort" id="sort" class="ml-4 px-4 py-2 border border-gray-300 rounded-md focus:outline-none"
                        onchange="window.location.href='{{ route('products.sale') }}?sort='+this.value">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="md:col-span-1">
                <div class="bg-gray-50 p-6 rounded-lg shadow-sm">
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('products.sale') }}" 
                               class="{{ !request('category') ? 'text-blue-600 font-medium' : 'text-gray-600 hover:text-blue-600' }}">
                                All Categories
                            </a>
                        </li>
                        @foreach($categories as $cat)
                        <li>
                            <a href="{{ route('products.sale') }}?category={{ $cat->slug }}" 
                               class="{{ request('category') == $cat->slug ? 'text-blue-600 font-medium' : 'text-gray-600 hover:text-blue-600' }}">
                                {{ $cat->name }}
                            </a>
                        </li>
                        @endforeach
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('products.sale') }}{{ request('category') ? '?category=' . request('category') : '' }}" 
                        ...
                        <li>
                            <a href="{{ route('products.sale') }}?brand={{ $brand->slug }}{{ request('category') ? '&category=' . request('category') : '' }}" 
            ...
            <div class="md:col-span-3">
                        ...
                        <a href="{{ route('products.show', $product->slug) }}">
                            <div class="relative h-64">
                                @php
                                    ...
                                    $image = !empty($images) ? $images[0] : 'https://via.placeholder.com/300';
                                @endphp
                ...
                <div class="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <h3 class="text-xl font-medium text-gray-600 mb-2">No sale items found</h3>
                    <p class="text-gray-500">Check back later for more deals!</p>
                    <a href="{{ route('products.index') }}" class="inline-block mt-4 px-6 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600">
                        Browse All Products
                    </a>
                </div>
                @endif
